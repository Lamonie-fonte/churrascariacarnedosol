(() => {
  ...
  async function sendCode(){
    ...
    button.disabled=true;await db.auth.signInWithOtp({email,options:{shouldCreateUser:true,data:{full_name:"Administrador"}}});button.disabled=false;
  ...
  function orderCard(o){return `<article class="order-card"><div><h3>Pedido #${o.order_number} • ${escapeHtml(o.customer_name)}</h3><p>${new Date(o.created_at).toLocaleString("pt-BR")} • ${escapeHtml(o.phone)} • ${o.order_type==="delivery"?"Entrega":"Retirada"}</p><strong>${money(o.total)}</strong></div><select data-order-id="${o.id}">${[["pending","Pendente"],["confirmed","Confirmado"],["preparing","Preparando"],["ready","Pronto"],["out_for_delivery","Saiu para entrega"],["completed","Concluído"],["cancelled","Cancelado"]].map(([v,n])=>`<option value="${v}" ${o.status===v?"selected":""}>${n}</option>`).join("")}</select></article>`;}
