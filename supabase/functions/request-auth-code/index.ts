...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: reserved, error: reserveError } = await supabase.rpc("reserve_auth_email_dispatch", { p_email: email });
    if (reserveError) throw reserveError;
    if (!reserved) return response(origin, { ok: true });

    const linkType = mode === "signup" ? "signup" : "magiclink";
    const { data: link, error: linkError } = await supabase.auth.admin.generateLink({
      type: linkType,
      email,
      options: mode === "signup" ? { data: { full_name: fullName } } : undefined,
    });
    if (linkError || !link?.properties?.email_otp) return response(origin, { ok: true });

    const { data: mailConfig, error: configError } = await supabase.rpc("get_auth_mail_config");
    ...
    console.error("request-auth-code", error);
