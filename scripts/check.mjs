...
const authResendMigration = await readFile(new URL("supabase/migrations/20260903182530_auth_resend_race_fix.sql", root), "utf8");
...
if (!verifyAuthFunction.includes('type: recovery ? "recovery" : "magiclink"') || !verifyAuthFunction.includes("freshLink.properties.hashed_token")) throw new Error("A verificação ainda depende do token invalidado por reenvio.");
if (/set used_at = coalesce\(used_at, now\(\)\)/.test(authResendMigration) || !authResendMigration.includes("and code_hash = v_code_hash")) throw new Error("O reenvio ainda pode invalidar um código atrasado.");
...
if ((storeHtml.match(/data-password-toggle=/g)||[]).length !== 3 || !storeHtml.includes('id="confirmPassword"') || !storeHtml.includes('id="authPassword"')) throw new Error("Os campos de senha não têm confirmação e botão de visualização.");
...
if (!app.includes("auth.signInWithPassword") || !admin.includes("auth.signInWithPassword") || !adminHtml.includes('id="adminPasswordLogin"')) throw new Error("O acesso com e-mail e senha não está conectado na loja e no painel.");
if (!app.includes('currentTarget?.id === "resendCodeButton"')) throw new Error("O botão Reenviar código não bloqueia novos toques durante o envio.");
