...
const icons = await readFile(new URL("assets/icons.js", root), "utf8");
...
if (!styles.includes("min-width:0;min-height:0;max-width:100%;max-height:100%;object-fit:contain")) throw new Error("As imagens dos cartões ainda podem ultrapassar sua moldura.");
if (!styles.includes("grid-template-columns:repeat(2,minmax(0,1fr));gap:10px")) throw new Error("A grade móvel não mantém dois produtos lado a lado.");
if (!styles.includes(".product-card{display:flex;min-width:0;flex-direction:column}")) throw new Error("Os cartões móveis não estão no formato vertical estável.");
if (!styles.includes(".dialog-product-image{position:relative;width:100%;min-height:0;height:auto;aspect-ratio:4/3;overflow:hidden}")) throw new Error("A imagem do produto aberto pode vazar sobre o conteúdo.");
if (!styles.includes(".dialog-product-image img{position:absolute;inset:0;width:100%;height:100%;max-width:100%;max-height:100%")) throw new Error("A imagem do produto aberto não está presa ao quadro móvel.");
if (!styles.includes(".product-dialog .add-cart-button{min-height:48px;padding:0 8px") || !styles.includes("white-space:nowrap")) throw new Error("O botão de adicionar pode quebrar em duas linhas no celular.");
...
if (!storeHtml.includes('/assets/icons.js') || !adminHtml.includes('/assets/icons.js') || !icons.includes("window.svgIcon") || !icons.includes("installSvgIcons")) throw new Error("A biblioteca profissional de ícones SVG não está carregada em todas as telas.");
const interactiveSource = [storeHtml, adminHtml, app, admin].flatMap(source => source.split("\n").filter(line => /<(?:button|a)\b|data-order-action|data-address-action/.test(line))).join("\n");
if (/📷|🖼|📲|📄|🗺|🧾|👁|🛵|🏪|🔥|🍽|👥/.test(interactiveSource)) throw new Error("Ainda existe emoji em um botão interativo.");
if (!app.includes('svgIcon("whatsapp")') || !admin.includes('svgIcon("edit")') || !storeHtml.includes('data-svg-icon="camera"')) throw new Error("Os botões principais ainda não usam o padrão SVG.");
console.log(`OK: ${catalog.length} produtos, ${expected.size} imagens, ${options.length} opções, carrinho profissional, finalização fixa, foto de perfil, e-mail rápido, ícones SVG, histórico, WhatsApp, PDF, mapa e painel administrativo.`);
